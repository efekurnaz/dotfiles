...
vim.api.nvim_create_autocmd("InsertCharPre", {
	...
	callback = function()
		...
		if char == " " then
			...
			if before_cursor:match("{{%s*$") or before_cursor:match("{%%%s*$") or before_cursor:match("|%s*$") then
...
vim.keymap.set(
	"n",
	"][",
	"/{% end\\(if\\|for\\|case\\|capture\\|unless\\|block\\) %}<CR>",
...
vim.keymap.set(
	"v",
	"<leader>lc",
	':lua wrap_in_liquid_tag("comment")<CR>',
	{ buffer = true, desc = "Wrap in comment block" }
)
vim.keymap.set("v", "<leader>lr", ':lua wrap_in_liquid_tag("raw")<CR>', { buffer = true, desc = "Wrap in raw block" })
vim.keymap.set(
	"v",
	"<leader>lp",
	':lua wrap_in_liquid_tag("capture")<CR>',
	{ buffer = true, desc = "Wrap in capture block" }
)
